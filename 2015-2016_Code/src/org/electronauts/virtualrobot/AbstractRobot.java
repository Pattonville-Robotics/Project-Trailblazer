...
public abstract class AbstractRobot implements Runnable {
	 ...
	 *
	 * @param abstractSensor the sensor to be added to the robot
	 */
	 ...
	 * Gets the bounds.
	 *
	 * @param scale the multiplier to scale the bounds up
	...
	public Motor getMotor(final MotorData motorData) {
		for (final Motor motor : this.motors)
			if (motor.getMotorData() == motorData)
				return motor;
	 ...
	 *
	 * @param motor the requested motor
	 ...
	 *
	 * @param sensorType the sensor type to be requested
	 ...
	 * @param rpm1   the first RPM
	 * @param rpm2   the second RPM
	 */
