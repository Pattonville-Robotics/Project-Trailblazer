...
public class MathUtil {
	...
	public static Point2D getIntersectionPoint(final Point2D p1, final Point2D p2, final Point2D p3, final Point2D p4) {
		...
		if (denominator == 0)
			...
			final Point2D.Double p = new Point2D.Double((top1 * (p3.getX() - p4.getX()) - (p1.getX() - p2.getX()) * top2) / denominator, (top1 * (p3.getY() - p4.getY()) - (p1.getY() - p2.getY()) * top2) / denominator);
			if (MathUtil.isInRange(p.x, p1.getX(), p2.getX()) && MathUtil.isInRange(p.x, p3.getX(), p4.getX()) && MathUtil.isInRange(p.y, p1.getY(), p2.getY()) && MathUtil.isInRange(p.y, p3.getY(), p4.getY()))
				return p;
			else
				return null;
		}
